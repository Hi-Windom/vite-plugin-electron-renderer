...
export default function useNodeJs(options: UseNodeJsOptions = {}): Plugin[] {
  ...
  const pluginResolveId: Plugin = {
    name: '@sillot/vite-plugin-electron-renderer:use-node.js[resolveId]',
    // Bypassing Vite's builtin 'vite:resolve' plugin
    enforce: 'pre',
    resolveId(source) {
      if (env.command === 'serve' || /* 🚧-① */pluginResolveId.api?.isWorker) {
        if (ESM_deps.includes(source)) return // processed by @sillot/vite-plugin-electron-renderer:optimizer
        if (builtins.includes(source)) return prefix + source
      }
    },
  }
  const plugin: Plugin = {
    name: '@sillot/vite-plugin-electron-renderer:use-node.js',
